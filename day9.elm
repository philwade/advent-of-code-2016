...
recursiveWalkInput input =
    if String.contains "(" input then
        recursiveWalkInput <| walkInput (String.split "" input) ""
    else
        input

walkInput : List String -> String -> String
walkInput input start =
    case List.head input of
        Just "(" ->
            let
                (instruction, set) = getInstruction (getRest input) ""
                (newStr, rest) = doInstruction instruction set
            in
                walkInput rest (start ++ newStr)
        Just char -> walkInput (getRest input) (start ++ char)
        Nothing -> start

doInstruction : Instruction -> List String -> (String, List String)
doInstruction instruction set =
    let
...
getInstruction : List String -> String -> (Instruction, List String)
getInstruction input start =
    case List.head input of
        Just ")" -> (parseInstruction start, getRest input)
        Just char -> getInstruction (getRest input) (start ++ char)
        Nothing -> Debug.crash "somehow hit a non terminated instruction"

parseInstruction : String -> Instruction
parseInstruction rawInstruction =
    let
        parts = String.split "x" rawInstruction
        grabInt a = case String.toInt a of
                        Ok n -> n
                        _ -> Debug.crash "bad integer"
    in
        case parts of
            amount::multiplier::_ -> Instruction (grabInt amount) (grabInt multiplier)
            _ -> Debug.crash ("bad instruction " ++ rawInstruction)


type alias Instruction = { take: Int
                         , multiplier: Int
                         }
