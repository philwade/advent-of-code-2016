...
How many IPs in your puzzle input support
-}

partOneOutput = countValid input valid
...
partTwoOutput = countValid input validSsl

main = div []
       [ text (toString partOneOutput)
       ]

type alias Ip = { abbas: List IpPart
                , hypernetSeqs: List IpPart
                }

type IpPart = Abba String | Hyper String

breakUpInput : String -> Ip
breakUpInput inputLine =
    let
        pairs = Regex.split Regex.All (regex "\\[|\\]") inputLine
        (_, ip) = List.foldl pairFold (1, Ip [] []) pairs

    in
        ip

pairFold : String -> (Int, Ip) -> (Int, Ip)
pairFold item (index, ip) =
    if index % 2 == 0 then
        (index + 1, { ip | hypernetSeqs = (List.reverse ((Hyper item) :: ip.hypernetSeqs)) })
    else
        (index + 1, { ip | abbas = (List.reverse ((Abba item) :: ip.abbas)) })

hasAbba : List String -> Bool
hasAbba input =
    let
        set = List.take 4 input
    in
        if List.length set > 3 then
            let
                front = List.take 2 set
                back = List.drop 2 set
            in
                if front == List.reverse back
                    && not (back == front) then
                    True
                else
                    hasAbba (List.drop 1 input)
        else
            False

findAbas : List (List String) -> List String -> List (List String)
findAbas abas str =
    if List.length str > 2 then
        let
            set = List.take 3 str
        in
            if isAba set then
                findAbas (set :: abas) (List.drop 1 str)
            else
                findAbas abas (List.drop 1 str)
    else
        abas

validSsl : Ip -> Bool
validSsl { abbas, hypernetSeqs } =
    let
        rawBabs = List.foldl sslFold [] abbas
        babs = List.map makeBab rawBabs
    in
        checkBabs babs hypernetSeqs

sslFold : IpPart -> List (List String) -> List (List String)
sslFold singleAbba babs =
    case singleAbba of
        Abba str -> findAbas [] (String.split "" str) ++ babs
        _ -> []

checkBabs : List String -> List IpPart -> Bool
checkBabs babs hypers =
    case List.head hypers of
        Just hyper ->
            case hyper of
                Hyper hyp ->
                    if validBab babs hyp then
                        True
                    else
                        checkBabs babs (List.tail hypers |> Maybe.withDefault [])
                _ ->
                    checkBabs babs (List.tail hypers |> Maybe.withDefault [])
        Nothing -> False

isAba : List String -> Bool
isAba str =
    case str of
        a::b::c::_ -> not (a == b) && a == c
        _ -> False

makeBab : List String -> String
makeBab aba =
    case aba of
        a::b::_ -> String.join "" (b::a::b::[])
        _ -> ""

validBab : List String -> String -> Bool
validBab babs hyper =
    case List.head babs of
        Just bab ->
            case String.contains bab hyper of
                True -> True
                False -> validBab (List.tail babs |> Maybe.withDefault []) hyper
        Nothing -> False

badHyper : List String -> Bool
badHyper input = hasAbba input

-- given a list of Hyper, will return True if they invalidate an IP
badHypers : List IpPart -> Bool
badHypers hypers =
    case List.head hypers of
        Just (Hyper str) ->
            let
            ...
            in
                if badHyper pieces then
                    True
                else
                    badHypers (List.tail hypers |> Maybe.withDefault [])
        Just (Abba str) ->
            True
        Nothing -> False

goodAbbas : List IpPart -> Bool
goodAbbas abbas =
    case List.head abbas of
        Just (Abba str) ->
            let
                pieces = String.split "" str
            in
                if hasAbba pieces then
                    True
                else
                    goodAbbas (List.tail abbas |> Maybe.withDefault [])
        Just (Hyper str) ->
            False
        Nothing -> False
...
valid : Ip -> Bool
valid { abbas, hypernetSeqs } =
    not (badHypers hypernetSeqs) && goodAbbas abbas

countValid : String -> (Ip -> Bool) -> Int
countValid input validator =
    String.split "\n" input
        |> List.map breakUpInput
        |> List.filter validator
        |> List.length

sampleInput = """abba[mnop]qrst
abcd[bddb]xyyx
aaaa[qwer]tyui
ioxxoj[asdfgh]zxcvbn"""

input = """Lyqbzgtboxxzpwr[kljvjjkjyojzrstfgrw]obdhcczonzvbfby[svotajtpttohxsh]cooktbyumlpxostt
